...
export class DynamicChartComponent extends WidgetComponent {
  ...
  private initialChart(): void {
    ...
    chart.plotContainer.children.push(
      am5.Label.new(this._root, {
        x: 10,
        y: 10,
        text: this.config['view'].label?.title,
        fill: am5.color(0xffffff),
      })
    );
    const nowDate = new Date().getTime();
    if (this.config['view'].scrollbarY.enable) {
      let scrollbarY = chart.set(
        'scrollbarY',
        am5xy.XYChartScrollbar.new(this._root, {
          orientation: 'vertical',
          width: this.config['view'].scrollbarY.width || 20,
        })
      );
    }
    ...
    if (this.config['view'].scrollbarX.enable) {
      scrollbarX = chart.set(
        'scrollbarX',
        am5xy.XYChartScrollbar.new(this._root, {
          orientation: 'horizontal',
          height: this.config['view'].scrollbarX.height || 40,
        })
      );
      this.config['view'].scrollbarX.position === 'bottom'
    ...
    this.config['content'].forEach((content: any) => {
        ...
        .pipe(
          map(history => {
            ...
            series.bullets.push(() => {
              return am5.Bullet.new(self._root, {
                ...
                sprite: am5.Circle.new(self._root, {
                  radius: this.config!['view'].bullet.size || 2,
            ...
            if (
              ...
              sbDateAxis &&
              sbValueAxis &&
              this.config['view'].scrollbarX.type === 'data'
            ) {
            ...
            if (this.config['view'].target.enabled) {
              const seriesRangeDataItem: any = yAxis.makeDataItem({
                value: this.config['view'].target.value || 20,
                endValue: 0,
              });
              ...
              seriesRangeDataItem.get('label').setAll({
                location: 0,
                visible: true,
                text: this.config['view'].target.title,
                inside: true,
                centerX: 0,
                centerY: am5.p100,
                fontWeight: 'bold',
