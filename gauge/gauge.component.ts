...
interface IGaugeChartUpdateItem {
  cols: number;
  content: {
    config: IGaugeChart;
    name: string;
...
export class GaugeComponent
  extends SharedFeaturesBaseComponent<IGaugeChart>
  implements AfterViewInit, OnDestroy
{
  ...
  updateGauge(item: IGaugeChartUpdateItem) {
    const config: IGaugeChart = item.content.config || this.config;
    if (this._root) {
      this._root.dispose();
    }
  ...
  override onDestroy() {
    ...
    // clearInterval(this._bringToLife);
  }
}
