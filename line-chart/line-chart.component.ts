...
import {ILineChart} from "../../../utils/models/line.interface";
...
export class LineChartComponent extends SharedFeaturesBaseComponent<ILineChart> implements AfterViewInit, OnDestroy {
  /**
   * @deprecated
   */
  ...
  ngAfterViewInit() {
    ...
    this.safeObservable(this.store.select(getDashboardItem(this.id))).pipe(
      map(item => {
        ...
        let chart = this._root.container.children.push(am5xy.XYChart.new(this._root, {
        ...
        let cursor = chart.set("cursor", am5xy.XYCursor.new(this._root, {
          behavior: "none",
        }));
        ...
        let xAxis = chart.xAxes.push(am5xy.DateAxis.new(this._root, {
          baseInterval: {timeUnit: "day", count: 1},
          renderer: am5xy.AxisRendererX.new(this._root, {}),
          tooltip: am5.Tooltip.new(this._root, {})
        }));

        let yAxis = chart.yAxes.push(am5xy.ValueAxis.new(this._root, {
          renderer: am5xy.AxisRendererY.new(this._root, {})
        }));

        let series = chart.series.push(am5xy.LineSeries.new(this._root, {
          name: "Series",
          xAxis: xAxis,
          yAxis: yAxis,
        ...
        if (config.view.scroll.enable) {
          let scrollbar = chart.set("scrollbarX", am5xy.XYChartScrollbar.new(this._root, {
            orientation: "horizontal",
            height: 20
          }));
          if (config.view.scroll.type === "sb") {
            let sbDateAxis = scrollbar.chart.xAxes.push(am5xy.DateAxis.new(this._root, {
              baseInterval: {
                timeUnit: "day",
                count: 1
              },
            ...
            let sbSeries = scrollbar.chart.series.push(am5xy.LineSeries.new(this._root, {
              valueYField: "value",
              valueXField: "date",
              xAxis: sbDateAxis,
              yAxis: sbValueAxis
